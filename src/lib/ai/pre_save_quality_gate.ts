...
function buildPreSaveSystemPrompt(): string {
  return [
    ...
    '10. Do not punish valid canon-backed worldbuilding terms or intentional mystery.',
    '11. Revise only affected spans; do not flatten atmosphere, mystery, character voice, competence, or genre register.',
    'Return JSON only.',
  ].join('\n');
}
function buildPreSaveUserPrompt(opts: PreSaveQualityGateOptions): string {
  ...
  return [
    `Project: ${opts.project.title || 'Untitled'}`, `Genre: ${opts.project.genre || 'Unknown'}`, `Writing style: ${opts.project.writingStyle || opts.project.tone || 'Not specified'}`, `Current chapter: ${currentChapterNumber}${opts.chapterTitle ? ` - ${opts.chapterTitle}` : ''}`, opts.chapterSummary ? `Draft summary: ${opts.chapterSummary}` : '', chapterIntent ? `Chapter intent: ${[chapterIntent.title, chapterIntent.summary, chapterIntent.focus].filter(Boolean).join(' | ')}` : '', '',
    ...
    'Semantic clarity audit rule:', 'Mystery may obscure facts but not sentence meaning. Flag semantic_obscurity or pseudo_coinage when meaning cannot be stably reconstructed in plain Vietnamese without adding information.', '',
    'Previous chapter tail/summary:', '"""', clampTail(previousSource || 'No previous chapter.', MAX_PREVIOUS_CHAPTER_CHARS), '"""', '', 'Whole-story context:', '"""', storyContext || 'No additional chapter context.', '"""', '', 'Generated chapter draft to check before saving:', '"""', opts.chapterContent, '"""', '', 'Return this JSON shape:',
    JSON.stringify({ approved: true, originalScore: 0, revisedScore: 0, issues: [{ type: 'proceduralized_characterization', severity: 'medium', description: 'short issue', fix: 'short fix' }], appliedChanges: ['short change summary'], revisedContent: 'full revised chapter text only' }),
  ].filter(Boolean).join('\n');
}
...
function normalizeIssueType(value: unknown): PreSaveQualityIssue['type'] { if (value === 'previous_continuity' || value === 'chapter_cohesion' || value === 'trait_literalization' || value === 'character_context_mismatch' || value === 'proceduralized_characterization' || value === 'semantic_obscurity' || value === 'pseudo_coinage') return value; return 'ai_tone'; }
