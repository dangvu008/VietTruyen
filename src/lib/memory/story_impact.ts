...
function summarizeDependencies(
  ...
  for (const dependency of filtered) {
    ...
    if (!existing.attributeKeys.includes(dependency.attributeKey)) {
      existing.attributeKeys.push(dependency.attributeKey);
    }
    if (dependency.context && !existing.reasons.includes(dependency.context)) {
      existing.reasons.push(dependency.context);
    }
...
function collectGraphImpact(
  ...
  const seedNodes = nodes.filter((node) =>
    node.refId === entityId || node.id.endsWith(`:${entityId}`)
  );
  const seedNodeIds = seedNodes.map((node) => node.id);
  if (seedNodeIds.length === 0 || maxDepth <= 0) {
  ...
  for (const edge of edges) {
    ...
    // Impact analysis is conservative: a dependency can propagate in either
    // direction even if the graph edge itself has a preferred direction.
  ...
  while (queue.length > 0) {
    ...
    for (const next of adjacency.get(current.nodeId) ?? []) {
      ...
      const edgesForNode = pathEdges.get(next.nodeId) ?? new Set<NarrativeEdgeType>();
      current.via.forEach((edgeType) => edgesForNode.add(edgeType));
      edgesForNode.add(next.edgeType);
      pathEdges.set(next.nodeId, edgesForNode);
...
/** Pure compiler used by tests and by the database-backed storyImpact facade. */
export function compileStoryImpact(input: {
  ...
  const impactedChapters = summarizeDependencies(input.dependencies, input.effectiveFromChapter);
  const graph = collectGraphImpact(
    input.entityId,
    input.nodes,
    input.edges,
    input.maxGraphDepth ?? 2,
  );
  const evidenceGaps: string[] = [];
  if (input.dependencies.length === 0) {
    evidenceGaps.push('No chapter dependency evidence was indexed for this entity/attribute.');
  }
  if (graph.seedNodeIds.length === 0) {
    evidenceGaps.push('No narrative-graph seed node was found for this entity.');
  }
...
/**
 * GitNexus-style blast-radius query for story canon changes.
 * It never mutates canon; callers must review the report before promotion.
 */
export async function storyImpact(request: StoryImpactRequest): Promise<StoryImpactReport> {
  ...
  // Stable ordering also makes reports deterministic and easier to diff in CI.
