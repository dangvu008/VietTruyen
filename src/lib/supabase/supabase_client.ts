 ...
 *
 * JSONB is intentionally represented as `unknown` here: the application owns the typed
 * NarrativeEraRegisterConfig contract, while the generated database file has not yet caught up.
 */
type GeneratedProjects = Database['public']['Tables']['projects'];
type ProjectsWithNarrativeEraRegister = {
  Row: GeneratedProjects['Row'] & { narrative_era_register: unknown };
  Insert: GeneratedProjects['Insert'] & { narrative_era_register?: unknown };
  Update: GeneratedProjects['Update'] & { narrative_era_register?: unknown };
