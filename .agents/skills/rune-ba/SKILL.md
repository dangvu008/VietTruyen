...
1. find files by pattern `.out-of-scope/*.md` — if directory absent, skip silently.
...
1. search file contents the codebase for evidence (function names, route handlers, schema definitions matching the asserted behavior).
...
## Next Step
→ Hand off to the rune-plan skill (consumes all 3 artifacts)
```
