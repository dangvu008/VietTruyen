...
- `test` (L2): write a regression test once the root cause is understood and the bug needs a stable reproduction artifact
...
### Step 5a: Agent Bug Autopsy Gate

<HARD-GATE>
If the bug was introduced by an agent, recurred across sessions, or was previously fixed by a human without an agent-facing diagnosis, do NOT hand off to `the rune-fix skill` yet.

First produce an autopsy record with:
1. **Symptom**: exact error or wrong behavior
2. **Cause**: what the agent misunderstood or generated incorrectly
3. **Reproduction**: command, test, or user path that triggers it
4. **Prevention**: rule, test, or project-doc note that would stop recurrence
5. **Fix Direction**: what `the rune-fix skill` should change after memory is written

Human patching without this record is a learning-loop failure: the code may be fixed, but the agent will repeat the same mistake later.
</HARD-GATE>

Append the autopsy record to `.rune/debug/knowledge-base.md` before Step 5b. If a regression test can reproduce the issue, call `the rune-test skill` before `the rune-fix skill` so the failure becomes executable memory, not just prose.
...
10. For agent-created, recurring, or human-patched bugs: MUST write an autopsy record before handing off to fix
...
| Agent-created bug is fixed without an autopsy record | CRITICAL | Step 5a blocks handoff to fix until symptom, cause, reproduction, prevention, and fix direction are captured |
...
- Agent-created or recurring bugs have an autopsy record written before fix handoff
