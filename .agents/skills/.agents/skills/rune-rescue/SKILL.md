...
- `context-engine` (L3): monitor context pressure between long-running rescue phases
- `scope-guard` (L3): verify each rescue session stays within the declared module blast radius
- `neural-memory` (L3): phase start + phase end — recall past refactoring patterns, capture new ones
