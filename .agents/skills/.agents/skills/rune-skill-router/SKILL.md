...
<MUST-READ path="references/instruction-precedence.md" trigger="before resolving conflicts between user prompt, repo rules, active modes, and skill contracts"/>
...
**Escape hatch**: If request is clearly trivial (< 5 LOC change, single-line fix, or user requests speed), classify as CODE_CHANGE but cook may activate Fast/Nano Mode only if the task stays above its minimum risk floor.
...
### Step 0.4 — Instruction Precedence Gate

Before routing or honoring a shortcut, apply `references/instruction-precedence.md`.

1. Classify the instruction conflict domain: `safety`, `architecture`, `workflow`, `scope`, or `presentation`
2. Treat user prompts as the source of truth for the task goal and output shape
3. Treat routed skills and repo contracts as the source of truth for mandatory workflow gates
4. Treat active modes as behavioral hints, not absolute law
5. Reject shortcuts like "skip routing", "no process", or "just write it" when request type is FULL enforcement
6. If the conflict is only about tone, audience, or formatting, prefer the user's inline instruction
...
6. **Apply the precedence contract** — user intent sets the goal, but routed workflow gates still apply
...
| Auto / autopilot / autonomous / "do it all" / "làm hết" / "đi ngủ" | `the rune-autopilot skill` ⚡Pro | Autonomous multi-session execution (requires approved plan + Pro tier installed) |

**Default route**: If unclear, route to `the rune-cook skill`. Cook handles 70% of all requests.

> **Pro skill note**: `the rune-autopilot skill` requires `@rune-pro` installed. If not available, fall back to `the rune-cook skill` with the approved plan and inform user that autopilot is a Pro feature.
...
8. MUST apply `references/instruction-precedence.md` when user shortcuts conflict with repo or skill governance
9. MUST NOT let inline requests bypass mandatory routing or quality gates for FULL-enforcement tasks
...
**User shortcut handling**: If user explicitly says "skip routing", "just write it", or "no process", treat that as a speed preference, not as permission to bypass routing. For FULL enforcement tasks, routing still applies. For LITE tasks with no matching skill, answer directly.
</HARD-GATE>
