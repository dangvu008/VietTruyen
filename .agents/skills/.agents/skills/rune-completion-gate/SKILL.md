...
<MUST-READ path="references/instruction-precedence.md" trigger="before deciding whether a missing step is an invalid shortcut or an approved omission"/>
...
### Step 1a — Collect Governing Context

Before validating claims, determine what the workflow actually promised to do:

1. Apply `references/instruction-precedence.md`
2. Read the active governing artifacts when present:
   - approved plan or phase file
   - requirements document
   - explicit documented skip reasons
   - active rigor path if the orchestrator announced one
3. Distinguish between:
   - a valid omission allowed by rigor or approved artifacts
   - an invalid shortcut where the agent skipped a required gate
4. Treat user inline requests as authoritative for output shape and scope intent, but not for bypassing required evidence
5. If the agent claims more than the approved scope/artifacts cover, mark that as a scope mismatch candidate for Step 3
...
| "implemented" | Files created/modified matching the approved scope or plan | Write/Edit tool calls vs approved artifacts |
...
3. Check for **scope mismatch** — does the evidence prove the SPECIFIC claim or a broader/narrower version?
   - If claim exceeds approved scope/artifacts, mark CONTRADICTED or UNCONFIRMED depending on evidence quality
...
6. MUST apply `references/instruction-precedence.md` before treating a skipped step as a failure or an accepted omission
