...
- `ba` (L2): clarify vague or business-heavy requirements before locking the implementation plan
...
- `problem-solver` (L3): structured reasoning when the planning problem spans multiple competing constraints
- `sequential-thinking` (L3): complex architecture with many trade-offs
- `test` (L2): shape test strategy and reference regression targets for each phase
- `scope-guard` (L3): check that proposed phase boundaries align with actual file ownership and blast radius
- `neural-memory` (L3): recall past architectural decisions on similar problems before finalizing the plan
...
<MUST-READ path="references/instruction-precedence.md" trigger="before resolving conflicts between user prompt, requirements, approved scope, and repo planning rules"/>
...
### Step 1.5 — Instruction Precedence Alignment

Before decomposing work:

1. Apply `references/instruction-precedence.md`
2. Treat the user's current request as the source of truth for the intended outcome
3. Treat requirements, contracts, and existing approved artifacts as the source of truth for in-scope deliverables and non-negotiable constraints
4. If the user asks for a shortcut that changes scope, assumptions, or required gates, convert that into either:
   - a scope amendment to be approved explicitly, or
   - a planning note that the shortcut is not allowed by the current governing artifacts
5. If a follow-up request conflicts with the current approved scope, do not silently absorb it into the same plan
...
If the user adds a new deliverable or changes scope during review:

- treat it as a scope decision, not a phrasing preference
- revise the master plan summary
- get approval again before writing or updating phase files
...
15. MUST apply `references/instruction-precedence.md` when inline instructions conflict with requirements, contracts, or approved scope
...
chain_metadata:
  ...
  suggested_next:
    ...
    - skill: "the rune-autopilot skill"
