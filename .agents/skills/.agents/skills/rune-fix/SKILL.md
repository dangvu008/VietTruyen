...
<HARD-GATE>
If the issue was introduced by an agent, has recurred across sessions, or was previously patched by a human without a recorded diagnosis, do NOT apply the fix directly.
Route to `the rune-debug skill` first and require an autopsy record in `.rune/debug/knowledge-base.md` with symptom, cause, reproduction, prevention, and fix direction.
Fixing without that record repeats the failure pattern: the code changes, but the agent learns nothing.
</HARD-GATE>
...
- `logic-guardian` (L2): check business-rule changes before finalizing risky fixes
...
- If the request is agent-created, recurring, or human-patched without diagnosis → call `the rune-debug skill` for autopsy before proceeding
...
9. MUST NOT fix agent-created, recurring, or human-patched bugs until debug has captured the autopsy record
...
| Agent Learning Gate | For agent-created/recurring/human-patched bugs: autopsy record in `.rune/debug/knowledge-base.md` | Run the rune-debug skill first |
