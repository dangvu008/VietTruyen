...
- `context-engine` (L3): coordinate compaction timing and context pressure thresholds before checkpoint or save
- `neural-memory` (L3): extract cross-project learnings from local session state when patterns should persist semantically
...
- `logic-guardian` (L2): persist protected business-rule decisions into local state
