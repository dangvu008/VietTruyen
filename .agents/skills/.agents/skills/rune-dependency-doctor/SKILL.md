...
- `verification` (L3): run a lightweight post-update smoke verification when dependency changes are applied or recommended fixes alter lockfiles
...
3. PER-PACKAGE LOOP (CRITICAL → SECURITY → PATCH → MINOR, skip MAJOR):
   ...
   b. Call `the rune-verification skill` — run tests + build
