...
- `adversary` (L2): challenge risky decomposition plans before dispatch when streams affect security, payments, or shared contracts
...
- `scaffold` (L1): delegate greenfield project generation when a stream is a new project bootstrap
...
- `context-engine` (L3): monitor context pressure before and during multi-stream orchestration
- `verification` (L3): run final integration verification after merging workstreams
- `git` (L3): coordinate branch, merge, and PR hygiene across workstreams
- `logic-guardian` (L2): validate business-logic integrity when streams touch manifested logic files
