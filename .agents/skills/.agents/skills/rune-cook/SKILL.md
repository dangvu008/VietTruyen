...
3. You MUST write failing tests before implementation (Phase 3) unless the active rigor path omits Phase 3 or an approved artifact records the skip reason
...
- User can override upward: "full pipeline" forces Full
- User can request speed: "just do it" may prefer Nano/Fast, but NEVER below the minimum rigor floor set by risk signals, contracts, or protected domains
...
**Override**: User can say "full pipeline" or "cook feature" to force phases. User requests for speed only apply if Nano remains allowed by the risk floor.
...
**Override**: User can say "full pipeline" to force all phases even on small changes. Requests to skip more phases are valid only when the risk floor still permits Fast Mode.
...
Skip if: bug fix with clear repro steps | rigor is already Nano/Fast and user requested speed | fast mode + <10 LOC | hotfix chain active. Complexity revealed → escalate to `the rune-ba skill`.
...
<MUST-READ path="references/instruction-precedence.md" trigger="before honoring user shortcuts that conflict with risk, workflow gates, or repo contracts"/>
...
**Step 0.55 — Instruction Precedence Check**:
1. Apply `references/instruction-precedence.md` before honoring "quick", "just do it", "skip tests", or similar shortcuts
2. Let the user request influence urgency, output shape, or delivery style
3. Do NOT let user shortcuts reduce rigor below the minimum floor implied by risk signals, contracts, or routed workflow gates
4. If a shortcut is accepted only partially, announce the restriction briefly before proceeding
...
3. Re-invoke the upstream skill (`the rune-plan skill` for plan issues, `the rune-ba skill` for spec gaps) with the finding as context
...
| 2 | `scaffold` | L1 | Hand off greenfield project creation when task is project bootstrap, not feature work |
| 2 | `graft` | L2 | Port or adapt existing features from external repositories when task is clone/port-oriented |
| 2 | `mcp-builder` | L2 | Build MCP servers when task scope is tool/resource server creation |
...
| 4 | `context-engine` | L3 | Monitor context pressure during long-running implementation and compact safely when needed |
...
| 5 | `retro` | L2 | Retrospective when repeated loops or recovery patterns reveal process issues worth capturing |
...
| 6 | `docs` | L2 | Update project documentation when feature scope changes public behavior or usage |
| 7 | `journal` | L3 | Record architectural decisions |
| 7 | `git` | L3 | Generate semantic commits and branch/release metadata |
...
3. MUST write failing tests before implementation (TDD) unless the active rigor path or an approved artifact explicitly allows a documented skip
...
9. MUST apply `references/instruction-precedence.md` when user shortcuts conflict with workflow gates or risk floors
