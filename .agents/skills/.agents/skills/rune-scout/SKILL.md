...
Scout follows [`references/context-retrieval-contract.md`](../../references/context-retrieval-contract.md) when deciding whether to use local search or retrieval-first scanning.
...
### Phase 0: Choose Retrieval Mode

Before reading files, choose one mode:

- `local-search` for tiny, obvious, localized tasks
- `graph-first` for unfamiliar areas, multi-file work, or architectural discovery
- `hybrid-impact` for edits, reviews, refactors, or blast-radius mapping

If GitNexus is available, prefer `graph-first` or `hybrid-impact` for medium/large work.
If GitNexus is unavailable or stale, fall back to `local-search` and report lower confidence.
...
### Phase 2: Targeted Search (Retrieval-First)
...
If mode is `graph-first` or `hybrid-impact`:

1. Query GitNexus for ranked candidate files, symbols, and relevant edges
2. Verify top candidates locally before trusting them
3. Stop at 2-4 high-signal candidates unless the caller explicitly needs broader mapping
...
3. Read the file to examine the most relevant files

Read budget:

- `local-search`: max 6 files
- `graph-first`: max 4 files
- `hybrid-impact`: max 6 files, but only after ranked candidates are chosen
...
Scout's default is "minimum sufficient context" — not "use the whole read budget." Track saturation across Phase 2 searches:
...
**Why**: Without saturation detection, scout reads its full budget even when 3 files already contain everything needed. This wastes context tokens and delays the calling skill. Early saturation detection returns control faster.
...
4. If graph retrieval is active, compare local findings with graph findings and report mismatches as freshness risk
...
- **Fast**: Use the smallest mode that fits. Max 6 file reads per scan unless full-map generation is explicitly needed
- **Focused**: Only scan what is relevant to the request, not the entire codebase
- **Retrieval-first**: On medium/large work, prefer ranked candidate discovery before broad reads
...
- **Retrieval Mode**: [local-search | graph-first | hybrid-impact]
...
- **Why these files**: [why the selected set is sufficient]
- **Excluded context**: [what was intentionally not read]
...
|---|---|---|
| Reading all files instead of ranked retrieval | MEDIUM | Choose retrieval mode first, then enforce read budget |
...
| Trusting stale graph output as fact | HIGH | Report `Index Status`, verify top candidates locally, fall back when mismatch is high |
...
- Retrieval mode and index status reported explicitly
