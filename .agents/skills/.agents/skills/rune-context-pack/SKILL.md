...
Context-pack follows [`references/context-retrieval-contract.md`](../../references/context-retrieval-contract.md). It should package the minimum sufficient context, not the maximum available context.
...
2. **NARROW** — Prefer retrieval artifacts over raw conversation:
   - Use ranked candidate files if available
   - Use `ImpactSlice` boundaries if available
   - Exclude files and modules that were considered but are not needed
   - Record whether the packet came from `local-search`, `graph-first`, or `hybrid-impact`

3. **COMPRESS** — Reduce to essential information:
...
4. **STRUCTURE** — Format as a context packet (see Output Format)

5. **VALIDATE** — Check packet completeness:
...
### Why These Files
- `path/to/file.ts` — [why this file is in-scope]

### Excluded Context
- `path/to/other-file.ts` — [why it was intentionally excluded]
...
| `retrieval_mode` | enum | `local-search` / `graph-first` / `hybrid-impact` / `not-applicable` |
...
7. MUST explain why included files are in scope when file paths are present
8. MUST record excluded context when narrowing decisions were made
9. MUST prefer retrieval artifacts over broad conversational recap
...
| Packet contains every related file instead of minimum sufficient context | HIGH | Add `Why These Files` and `Excluded Context`, enforce budget |
| Stale graph-derived scope passed to subagent as fact | HIGH | Include `Index Status`, warn when scope came from stale retrieval |
...
- [ ] Included files are justified explicitly
- [ ] Excluded context is noted when narrowing occurred
...
- Retrieval mode and index status reported
