---
name: rune-onboard
description: "Auto-generate project context for AI sessions. Scans codebase, creates a platform-appropriate project context file (CLAUDE.md or AGENTS.md) plus .rune/ setup so every future session starts with full context."
---
...
Auto-generate project context for AI sessions. Scans the codebase and creates a platform-appropriate project context file plus `.rune/` state directory so every future session starts with full context. Saves 10-20 minutes of re-explaining per session on undocumented projects.

## Triggers

- `/rune onboard` — manual invocation on any project
- Called by `rescue` as Phase 0 (understand before refactoring)
- Auto-trigger: when no project context file exists in project root
...
- `cook` (L1): if no project context file found, onboard first

## Output Files

```
project/
├── AGENTS.md or CLAUDE.md # Platform-appropriate AI project context
└── .rune/
...
### Step 3.5 — Select Project Context File
Choose the target context file before writing anything:

- If `AGENTS.md` already exists, keep using `AGENTS.md`
- Else if `.cursor/`, `.windsurf/`, `.codex/`, `.opencode/`, `.agents/`, or `.ai/` exists at project root, use `AGENTS.md`
- Else if `CLAUDE.md` already exists, keep using `CLAUDE.md`
- Else if `.claude/` or `.claude-plugin/` exists, use `CLAUDE.md`
- Else default to `AGENTS.md` for new agent-neutral projects

Record the chosen filename as `[PROJECT_CONTEXT_FILE]` and reuse it in all later steps.

### Step 4 — Generate Project Context File
Write/create the file to create `[PROJECT_CONTEXT_FILE]` at the project root. Populate every section using data from Steps 2–3. Do not leave template placeholders — if data is unknown, write "unknown" or omit the section. Use the template below as the exact structure.

If `[PROJECT_CONTEXT_FILE]` already exists, Read the file to load it first, then merge — preserve any human-written sections (comments starting with `<!-- manual -->`) and update auto-detected sections only.
...
2. Check `[PROJECT_CONTEXT_FILE]` line count
3. If total MCP tools >80 or `[PROJECT_CONTEXT_FILE]` >150 lines, include a **Context Budget Advisory** in the Onboard Report:

```
### Context Budget Advisory
- **MCP tools loaded**: [count] across [N] servers
- **Project context size**: [N] lines in `[PROJECT_CONTEXT_FILE]`
- **Estimated baseline**: ~[N]k tokens before any work begins
- **Recommendation**: [specific advice — disable unused MCP servers, move heavy project details to `.rune/`]
```

**Skip if**: Total MCP tools ≤80 AND `[PROJECT_CONTEXT_FILE]` ≤150 lines (healthy baseline).
...
```bash
git add [PROJECT_CONTEXT_FILE] .rune/ && git commit -m "chore: initialize rune project context"
```
...
## Key Files
- Entry point: [project-relative path]
- Config: [project-relative paths]
- Routes/API: [project-relative paths]
```
...
### Generated
- `[PROJECT_CONTEXT_FILE]` (project configuration)
...
1. MUST scan actual project files — never generate the project context file from assumptions
2. MUST detect and respect existing project context content — merge, don't overwrite
...
|---|---|---|
| Project context file generated from README alone (no file scan) | CRITICAL | Step 1 MUST invoke scout — never skip actual file scanning |
...
- `[PROJECT_CONTEXT_FILE]` written (or merged) with all detected tech stack fields populated
...
|----------|--------|----------|
| Project AI config | Markdown | `[PROJECT_CONTEXT_FILE]` (project root) |
